...
class supplierClass:
    def __init__(self,root):
        ...
        self.root.focus_force()   # to highlight the window when it pops up
        #===========================================================================================================
        # All Variables===
        ...
        btn_search=Button(self.root,text="Search",command=self.search,font=("goudy old style",15),bg="blue",fg="white",cursor="hand2").place(x=950,y=79,width=100,height=28) #placing was an issue
        ...
        #===Buttons===
        btn_add=Button(self.root,text="Save",command=self.add,font=("goudy old style",15),bg="#2196f3",fg="white",cursor="hand2").place(x=150,y=370,width=110,height=35)
        btn_update=Button(self.root,text="Update",command=self.update,font=("goudy old style",15),bg="#4caf50",fg="white",cursor="hand2").place(x=270,y=370,width=110,height=35)
        btn_delete=Button(self.root,text="Delete",command=self.delete,font=("goudy old style",15),bg="#f44336",fg="white",cursor="hand2").place(x=390,y=370,width=110,height=35)
        btn_clear=Button(self.root,text="Clear",command=self.clear,font=("goudy old style",15),bg="#607d8b",fg="white",cursor="hand2").place(x=510,y=370,width=110,height=35)


        #===Employee_Details===

        emp_frame=Frame(self.root,bd=3,relief=RIDGE)
        emp_frame.place(x=700,y=120,width=350,height=350)
        ...
        self.supplierTable["show"]="headings"   #Else it will show a default blank heading

        self.supplierTable.column("invoice",width=90)
        self.supplierTable.column("name",width=100)
        self.supplierTable.column("contact",width=200)
...
#========================================================================
    def add(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                cur.execute("Select * from supplier where invoice=?",(self.var_sup_invoice.get(),)) #importance of , after getm
                row=cur.fetchone()
                if row!=None:
    ...
    def show(self):
        ...
        cur=con.cursor()
        try:
            cur.execute("select * from supplier") #we didnot commit it as we need it in runtime
            rows=cur.fetchall()
    ...
    def update(self):
        ...
        try:
            if self.var_sup_invoice.get()=="":
                ...
                row=cur.fetchone()
                if row==None:
                    messagebox.showerror("Error","Invalid no",parent=self.root)
                else:
